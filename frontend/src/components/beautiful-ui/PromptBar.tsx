...
const COMMANDS: Suggestion[] = [
  {
    id: "plan",
    label: "/plan scenes",
    detail: "Build or revise the shot plan",
    action: "command",
  },
  {
    id: "assemble",
    label: "/assemble film",
    detail: "Prepare the approved clips for assembly",
    action: "command",
  },
];
...
export function PromptBar({
  ...
  const setDraft = (update: React.SetStateAction<string>) =>
    setDrafts((current) => ({
      ...current,
      [draftKey]:
        typeof update === "function" ? update(current[draftKey] || "") : update,
    }));
  type AttachmentDraft = Array<
    AgentAttachment & { uploading?: boolean; error?: string }
  >;
  const [attachmentDrafts, setAttachmentDrafts] = useState<
    Record<string, AttachmentDraft>
  >({});
  const attachments = attachmentDrafts[draftKey] || [];
  const setAttachments = (update: React.SetStateAction<AttachmentDraft>) =>
  ...
  useImperativeHandle(ref, () => ({
    ...
    setDraft: (text, scope = draftKey) => {
      setDrafts((current) => ({ ...current, [scope]: text }));
      setMenu(null);
      input.current?.focus();
    },
  }));
  ...
  const suggestions = useMemo<Suggestion[]>(() => {
    const contextRows = contextOptions
      .filter(
        (option) =>
          menu !== "mentions" ||
          option.label
            .toLowerCase()
            .includes((draft.match(/@([^@]*)$/)?.[1] || "").toLowerCase()),
      )
      .slice(0, 8)
      .map((option) => ({
        id: option.id,
  ...
  const attach = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files).slice(
      ...
      try {
        ...
        setAttachments((items) =>
          items.map((item) => (item.id === id ? uploaded : item)),
        );
      } catch (error) {
        setAttachments((items) =>
          items.map((item) =>
  ...
  const choose = (suggestion: Suggestion) => {
    if (suggestion.action === "upload") {
      ...
      setDraft(
        (value) => `${value.replace(/@[^\s]*$/, "")}@${suggestion.label} `,
      );
    } else {
  ...
  return (
    ...
    >
      {starterSuggestions.length > 0 && (
        <div
          className="agent-prompt-suggestions"
      ...
      {menu && (
        <div
          ...
          role="listbox"
          aria-label={menu === "commands" ? "Commands" : "Project sources"}
        >
          {suggestions.length ? (
      ...
      <div className="agent-prompt-bar">
        {(context || contexts.length > 0 || attachments.length > 0) && (
          <div className="agent-prompt-meta">
            ...
            {attachments.map((attachment) => (
              <div
                key={attachment.id}
                className={`agent-prompt-attachment ${attachment.error ? "has-error" : ""}`}
              >
                ...
                <span title={attachment.error || attachment.name}>
                  {attachment.name}
                </span>
                <button
                  type="button"
        ...
        <div
          className={`agent-prompt-row ${allowExtras ? "" : "agent-prompt-row--plain"}`}
        >
          {allowExtras && (
            <button
              type="button"
              ...
              onClick={() =>
                setMenu((current) => (current === "sources" ? null : "sources"))
              }
            >
              <Plus size={17} />
            </button>
          ...
          <textarea
            ...
            onKeyDown={(event) => {
              ...
              if (
                ...
                event.preventDefault();
                choose(suggestions[activeOption] || suggestions[0]);
                return;
              }
              if (
                event.key === "Enter" &&
